...
IntakeSystem::IntakeSystem() : Subsystem("IntakeSystem") {
	...
	leftIntakeActuator = RobotMap::intakeSystemLeftIntakeActuator;
	rightIntakeActuator = RobotMap::intakeSystemRightIntakeActuator;
...
void IntakeSystem::ActuateLeftIntake(bool on) {
	if(on) {
		leftIntakeActuator->Set(DoubleSolenoid::kForward);
	} else {
		leftIntakeActuator->Set(DoubleSolenoid::kForward);
	}
}

void IntakeSystem::ActuateRightIntake(bool on) {
	if(on) {
		rightIntakeActuator->Set(DoubleSolenoid::kOff);
	} else {
		rightIntakeActuator->Set(DoubleSolenoid::kOff);
	}
}
