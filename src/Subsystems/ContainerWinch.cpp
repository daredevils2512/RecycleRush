...
void ContainerWinch::SetWinch(int winchNumber, bool output) {
	switch (winchNumber) {
		case 1:
			if (output) {
				containerPull1->Set(Relay::kOn);
			} else {
				containerPull1->Set(Relay::kOff);
			}
			break;
		case 2:
			if (output) {
				containerPull2->Set(Relay::kOn);
			} else {
				containerPull2->Set(Relay::kOff);
			}

			break;
		case 3:
			if (output) {
				containerPull3->Set(Relay::kOn);
			} else {
				containerPull3->Set(Relay::kOff);
			}
			break;
		default:
			containerPull1->Set(Relay::kOff);
